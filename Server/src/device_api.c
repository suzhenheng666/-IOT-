...
/**
 * 设备数据上报处理 - 接收并保存设备传感器数据
 */
void device_report_handler(struct bufferevent* bev, const http_request_t* req)
{
    if (strcmp(req->method, "POST") != 0) {
        send_json_error(bev, 405, "只支持 POST 方法");
        return;
    }
    
    if (!req->body || req->body_length == 0) {
        printf("[DEBUG] 设备上报：请求体为空，body=%p, body_length=%zu\n", 
               (void*)req->body, req->body_length);
        send_json_error(bev, 400, "请求体为空");
        return;
    }
    
    printf("[DEBUG] 设备上报：收到 %zu 字节的请求体\n", req->body_length);
    printf("[DEBUG] 设备上报：原始数据 = %.*s\n", (int)req->body_length, req->body);
    
    // 解析JSON数据
    cJSON* root = cJSON_Parse(req->body);
    if (!root) {
        printf("[ERROR] JSON 解析失败：%s\n", cJSON_GetErrorPtr());
        send_json_error(bev, 400, "无效的 JSON 格式");
        return;
    }
    ...
    {
        command = "turn_on_fan";
        printf("[警告] 温度超过 30℃，准备下发【开启风扇】指令！\n");
    } else {
        command = "turn_off_fan";
        printf("[正常] 温度安全，准备下发【关闭风扇】指令。\n");
    }

    char response_body[256];
    snprintf(response_body, sizeof(response_body),
        "{\"code\":0,\"msg\":\"data_received\",\"device_id\":\"%s\",\"command\":\"%s\"}",
        device_id,command);

    send_json_ok(bev, response_body);
